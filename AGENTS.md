...
このリポジトリで作業するときは、以下を守る。

- 作業開始時に `git status --short --branch` と `git fetch --prune` を実行し、追跡ブランチとの差分とリモート更新を確認する。
- 他端末の変更でリモートが進んでいる場合は、作業ツリーがクリーンであることを確認して `git pull --ff-only` など非破壊の方法で同期してから作業する。fast-forward できない場合は作業を止め、履歴を確認する。
- 変更は原則としてタスク単位の branch / worktree で行い、同じブランチを複数PCで同時編集しない。
- 別PCへ引き継ぐ前に変更をすべて確認して commit し、未コミット・未追跡の変更を残したまま引き継がない。
- 作業完了時は関連チェックを実行し、commit / push したうえで、引継ぎ先へリポジトリ名・ブランチ名・最新commit SHAを伝える。
- 競合や履歴の分岐を検知した場合は、勝手な上書き、`git reset --hard`、force pushを行わず、差分と履歴を確認して安全な統合方法を決める。
- マージと本番確認が完了した後に限り、使用中でないことを確認して不要なworktreeとローカル／リモートbranchを整理する。
