...
- 公開先の唯一の正本は中央台帳 `C:\Project\docs\cloudflare-targets.json`。`.github/deployment-platform.json` はGitHub側の対応表として台帳と照合する。対応表の `cloudflare.targetVerified=true`、登録Target名、Wrangler設定、accountId、作業場所を照合し、ローカルの `check-cloudflare-deployment.mjs --for-deploy` に加えて中央の `assert-cloudflare-target.ps1 -ForDeploy` が通る場合だけdeployできる。
