...
import { postToX, postToThreads, snsCredentialStatus, type SnsPostResult } from "../_lib/sns.js";
...
const postHandler = withAdmin({ method: "POST" }, async ({ res, body }) => {
  ...
  const skipped = (reason: string): SnsPostResult => ({ status: "skipped", error: reason });
  const [xResult, threadsResult] = await Promise.all<SnsPostResult>([
    wantX ? postToX(text) : Promise.resolve(skipped("not requested")),
    wantThreads ? postToThreads(text) : Promise.resolve(skipped("not requested")),
  ]);
