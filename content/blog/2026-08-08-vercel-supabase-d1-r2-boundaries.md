---
title: "Vercelを減らす前に知っておきたい：Supabase AuthとD1・R2をどう分けるか"
date: 2026-08-08
authorship_note: "この記事は、AIを整理・編集の補助として使い、運営者が内容を確認・編集しています。"
role: ブログ / AI活用・Webシステム設計
gen_by: 由井辰美 / AI相談
summary: Vercelの費用や運用を見直すときに、公開ページ、軽いサイトデータ、ファイル、ログイン、会員・業務データをどう分けるかを解説。Supabase AuthとRLSを残す場面、D1・R2が向く場面、ChatGPT SitesとCloudflare Workersを試す順番を、地域事業者や小規模チーム向けに整理します。
...
duration: 9分
goal: 自分のサイトのデータを、公開情報、軽量データ、ファイル、認証・権限、外部サービスの5つに分け、次に残す基盤と試せる基盤を判断できるようになる
---

「Vercelで公開している小さなサイトは、もっと軽くできないだろうか」

「ログインだけはSupabaseに残して、イベント情報やお知らせはもっと小さく持てないだろうか」

AIでサイトや業務ツールを作れるようになると、次に出てくるのがこの悩みです。画面を作ることは以前より速くなりました。だからこそ、どのデータをどこへ置くかを最初に分けないと、後から認証、権限、費用、引継ぎが絡み合います。

先に結論を言うと、私は次のように考えています。
...
たとえば、地域の講座やイベントを案内するサイトでも、実際には次の5つが混ざっています。
...
## D1・R2とSupabaseは、データの大きさではなく権限の複雑さで分ける
...
Supabase Authは「この人は誰か」を確認する仕組みです。さらにSupabase PostgresのRLS（行レベルセキュリティ）は、「その人が、このデータのどこまで見たり変えたりできるか」をデータベース側で守れます。
...
Vercelを完全にやめるか、すべてを移すか、の二択にする必要はありません。
...
新しいサイトや既存サイトを見直すときは、最初から一番大きな構成にしません。次の6段階で考えると、現場で判断しやすくなります。
...
<div class="publishing-cta">
  <strong>サイトが増えてきたら、「どのサービスを使うか」より先に、「何を預けるか」を整理します。</strong>
  <p>AI相談では、公開情報、会員・権限、ファイル、決済・予約、既存SaaSを分け、現場で続けられる構成と移行順序を一緒に設計します。</p>
