...
def decorate_art_direction(text):
    ...
    # A presentation-only layer; retain links, scripts, forms and content order.
    def theme(match):
        tag=match[0]
        classes=re.search(r'\bclass=([\"\x27])(.*?)\1',tag,re.S)
        names=classes[2].split() if classes else []
        if 'studio-paper' not in names:names.append('studio-paper')
        return attr(tag,'class',' '.join(names))
    text=re.sub(r'<body\b[^>]*>',theme,text,count=1)
    text=re.sub(r'\s*<link\b[^>]*href=[\"\x27]/design-system/studio/paper-design\.css[^\"\x27]*[\"\x27][^>]*>','',text)
    if '</head>' in text:
        text=re.sub(r'\s*</head>',lambda _: '<link rel="stylesheet" href="/design-system/studio/paper-design.css?v='+EDITORIAL_VERSION+'">\n</head>',text,count=1)
    return text
