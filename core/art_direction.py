...
def decorate_art_direction(text):
    # Remove only the retired presentation layer; keep content and scripts intact.
    text=re.sub(r'<link\b[^>]*>',lambda m:'' if re.search(r"href=[\"']\/design-system/studio/paper-design\.css(?:\?[^\"']*)?[\"']",m[0]) else m[0],text)
    def restore_body(match):
        return re.sub(r"(\bclass=[\"'])(.*?)([\"'])",lambda m:m[1]+' '.join(c for c in m[2].split() if c!='studio-paper')+m[3],match[0])
    text=re.sub(r'<body\b[^>]*>',restore_body,text)
