...
# Design QA — Soft Editorial Cloud（案1）

- source visual truth: `C:\Users\yui\.codex\visualizations\2026\08\01\019fbe2d-1765-76b3-9b4d-a54e69b0877b\ai-consult-design-qa\reference-selected.png`
- final implementation: `C:\Users\yui\.codex\visualizations\2026\08\01\019fbe2d-1765-76b3-9b4d-a54e69b0877b\ai-consult-design-qa\implementation-pc-final.png`
- normalized comparison board: `C:\Users\yui\.codex\visualizations\2026\08\01\019fbe2d-1765-76b3-9b4d-a54e69b0877b\ai-consult-design-qa\comparison-pc-final.png`
- additional evidence: `implementation-pc-packages.png`, `implementation-mobile-top.png`, `implementation-mobile-menu-open.png`, `implementation-mobile-salon-open.png`
- viewport / density: reference 1536 × 1024px、implementation 1440 × 1024 CSS px / DPR 1、mobile 390 × 844 CSS px / DPR 1
- states: desktop hero、desktop unified course menu、mobile hero、mobile menu open、mobile salon details open

## Comparison history

1. Pass 1 — P2: ヒーロー写真の上下余白が基準より大きく、コース開始位置が約56px早かった。
2. Pass 2 — 修正: ヒーローを746pxへ整え、写真を690pxへ拡大。コース開始位置を820pxへそろえた。
3. Pass 3 — 修正: 写真を右寄せ86%・1.18倍へ調整し、人物が主役になるトリミングへ変更。横はみ出し0を確認した。

## Required fidelity surfaces

- Background: アイボリーを基調に、左下のペリウィンクル曲線と控えめなピーチ点を継承。
- Surfaces: 白〜薄ラベンダーのフラットなカード、細い境界線、低い影で統一。
- Hierarchy: インクネイビーの見出し、コーンフラワーブルーの強調、CTAの順序を基準案と一致。
- Layout: PCはコピーと写真の2カラム、5コースは1段、サロンは5つ目のメニューとして直下へ統合。
- Continuity: 既存コピー、価格、時間、予約・資料リンク、詳細内容を削らず維持。

## Responsive and interactions

- Desktop 1440px: 5カード、カード高379〜442px、サロン閉状態212px、ページ横はみ出し0。
- Mobile 390px: 5カードを270px幅の横スクロールへ変更。ページ本体の横はみ出し0。
- Mobile menu: ボタンは44 × 108px、`aria-expanded=true`、`aria-hidden=false`、bodyスクロール固定、9リンク、明るい背景と濃色文字を確認。
- Salon details: summaryは1件。開状態で価値3件、基本情報4件、メリット8件、参加手順3件、画像2件の読み込みを確認。
- Browser console errors: desktop 0、mobile 0。

## Findings

対象範囲に未解決のP0 / P1 / P2はない。写真の人物構成は既存の実写素材を使うため基準生成画像と完全一致しないが、トリミング、密度、配色、余白、情報階層は同等の意図へ整えた。
