...
- 既存のAI相談向けリール資産の読みやすい型を継承し、仕事でAIを使う人向けの約25秒／5場面へ更新した。
- 音声は `Microsoft Nanami Neural（日本語・女性）`。親しみやすさと信頼感を保ち、通常速度で画面中央の全文を読む。
- BGMは外部音源やサンプルを使わずPython標準ライブラリだけで合成し、ナレーション中は約6dBダッキングする。
- FFmpeg `volumedetect` で生成済みナレーション、原BGM、入力ゲイン後bed、duck後BGMのナレーション区間RMSを実測する。今回の結果は narration -18.70 dBFS / music gain -13.10 dB / ducked BGM -49.20 dBFS / measured ducking 6.10 dB / voice lead 30.50 dB。
