...
def render_ai_agent_readiness_page(
    ...
    return "".join(
        [
            ...
            "<section id='addon-diagnostic' class='addon-diagnostic' aria-labelledby='addon-title'>",
            "<div class='addon-diagnostic__header'><p class='eyebrow'>OPTIONAL DEEP DIVE · 各6問・約2分</p>",
            "<h3 id='addon-title'>さらに深く診断（任意）</h3>",
            "<p>必要な方だけ、実装または組織導入の詰まりを深掘りできます。追加結果は別に表示し、10問の100点には加算しません。</p></div>",
            "<div id='addon-track-picker' class='addon-track-picker'>",
            "<button class='addon-track-card' type='button' data-addon-track='implementation'>",
            "<span class='addon-track-card__eyebrow'>作る・任せる・確かめる</span><strong>実装編 6問</strong>",
            "<span>コピペ利用、文脈、ルール、レビュー、テスト、実画面確認を振り返る</span><b>この6問を始める →</b></button>",
            "<button class='addon-track-card' type='button' data-addon-track='organization'>",
            "<span class='addon-track-card__eyebrow'>安全に導入し、価値へつなげる</span><strong>組織導入編 6問</strong>",
            "<span>利用制約、研修、安全、費用対効果、空いた時間、人の対話を振り返る</span><b>この6問を始める →</b></button>",
            "</div>",
            "<form id='addon-form' class='addon-form' hidden>",
            "<div class='assessment-progress'><span id='addon-progress-label'>質問 1 / 6</span>",
            "<progress id='addon-progress' max='6' value='0'>0 / 6</progress></div>",
            "<p id='addon-status' class='sr-only' aria-live='polite'></p>",
            "<fieldset id='addon-question-fieldset'>",
            "<legend><span id='addon-question-number'>Question 01</span><strong id='addon-question-prompt'>質問を読み込んでいます</strong></legend>",
            "<p id='addon-question-context' class='question-context'></p>",
            "<div id='addon-answer-options' class='answer-options'></div></fieldset>",
            "<div class='assessment-navigation'><button id='addon-previous' class='button button--secondary' type='button'>戻る</button>",
            "<button id='addon-next' class='button button--primary' type='button'>次へ</button></div></form>",
            "<section id='addon-result' class='addon-result' hidden aria-labelledby='addon-result-heading'>",
            "<p class='eyebrow'>OPTIONAL DIAGNOSTIC RESULT</p><h4 id='addon-result-heading'>追加診断の結果</h4>",
            "<div class='addon-result__score'><strong id='addon-result-percent'>0%</strong>",
            "<span id='addon-result-raw'>0 / 30点</span></div>",
            "<p id='addon-result-band' class='addon-result__band'></p><p id='addon-result-summary'></p>",
            "<section class='addon-result__next'><h5>まず整える2つ</h5><ol id='addon-next-actions'></ol></section>",
            "<section class='addon-result__courses'><h5>必要なら使える既存の相談・講習</h5>",
            "<p>AI相談が提供する任意の案内です。行動案は受講せずに試せます。</p>",
            "<div id='addon-course-options' class='addon-course-options'></div></section>",
            "<div class='addon-result-actions'><button id='copy-addon-result' class='button button--primary' type='button'>追加結果を含む相談メモをコピー</button>",
            "<button id='restart-addon' class='button button--secondary' type='button'>この6問をやり直す</button>",
            "<button id='choose-addon' class='text-button' type='button'>もう一方を選ぶ</button></div>",
            "<p id='addon-copy-status' class='copy-status' aria-live='polite'></p></section>",
            "<p class='addon-source'>質問は、YouTube動画「<a href='https://www.youtube.com/watch?v=92n-hUhRE58' target='_blank' rel='noopener noreferrer'>エンジニアなら、これくらいAI使えないとね。</a>」の論点を現場向けに再編集。動画の個別発言を正解や点数の基準にはしていません。</p>",
            "</section>",
            "</section>",
