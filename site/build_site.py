...
def render_index(payload: dict, genres: list[dict], is_live: bool = True) -> str:
    ...
    parts.append(f"<title>AI Watch Top{total} / {date} | AIスペシャリスト 由井辰美</title>")
    ...
    parts.append(_build_ogp("AIスペシャリスト 由井辰美 AI Watch", desc, SITE_URL + "/watch/index.html", kind="website"))
    ld = _build_jsonld("website", {}, "AIスペシャリスト 由井辰美 AI Watch", SITE_URL + "/watch/index.html")
    if ld:
    ...
    parts.append("<header>")
    parts.append("<h1>AIスペシャリスト 由井辰美</h1>")
    ...
    if not items:
        ...
        parts.append("<footer>AIスペシャリスト 由井辰美</footer></div></body></html>")
        return "".join(parts)
    ...
    parts.append(render_support_sns_section(load_support_sns()))
    parts.append("<footer>AIスペシャリスト 由井辰美 / Generated by Claude</footer>")
    parts.append("</div>")
...
def render_archive(dates: list[str]) -> str:
    ...
    parts.append("<!doctype html><html lang='ja'><head><meta charset='utf-8'>" + FAVICON_HEAD_HTML)
    parts.append("<meta name='viewport' content='width=device-width,initial-scale=1'>")
    parts.append("<title>AIスペシャリスト 由井辰美 — AI Watch 過去ログ</title>")
    parts.append(f"<style>{MASTER_CSS}</style></head><body><div class='container'>")
    parts.append(ADMIN_BUTTON_HTML)
...
def _build_ogp(title: str, description: str, page_url: str, kind: str = "article") -> str:
    ...
    return "".join([
        ...
        "<meta property='og:site_name' content='AIスペシャリスト 由井辰美'>",
...
def render_content_page(title: str, meta: dict, body_html: str, nav_html: str, page_path: str = "", kind: str = "") -> str:
    ...
    parts.append("<!doctype html><html lang='ja'><head><meta charset='utf-8'>" + FAVICON_HEAD_HTML)
    parts.append("<meta name='viewport' content='width=device-width,initial-scale=1'>")
    parts.append(f"<title>{html.escape(title)} | AIスペシャリスト 由井辰美</title>")
    desc = str(meta.get("summary") or "")
    if desc:
        parts.append(f"<meta name='description' content='{html.escape(desc, quote=True)}'>")
...
def build_lectures() -> int:
    ...
    if sections:
        ...
        (out_dir / "index.html").write_text(
            render_content_page("受講資料 ディレクトリ", {"summary": "AIスペシャリスト 由井辰美の受講資料・補助教材・外部リソースのディレクトリ"}, body_html, nav, page_path="lectures/index.html"),
            encoding="utf-8",
        )
    return count
...
def build_blog() -> int:
    ...
    if items:
        parts = [
            ...
            "<p>AIスペシャリストの実践ブログです。講習・制作・AI活用の現場から、実際に使える視点を残していきます。</p>",
            "<div class='tr-grid'>",
        ]
        for item in items:
        ...
        (out_dir / "index.html").write_text(
            render_content_page(
                "ブログ",
                {"summary": "AIスペシャリスト 由井辰美のブログ一覧。Codex、Claude Code、生成AI活用、AIコーディングの実践記録。"},
                "".join(parts),
                nav,
...
def build_portfolio_page() -> bool:
    ...
    meta = {
        "summary": "AIスペシャリスト由井辰美が制作・運営している実績サイト一覧。カテゴリ・技術スタック・公開年で絞って俯瞰できる。",
    }
...
def build_profile_page() -> bool:
    ...
    redirect = (
        ...
        "<title>講師紹介・経歴 — AIスペシャリスト 由井辰美</title>"
