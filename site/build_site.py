...
def render_index(payload: dict, genres: list[dict]) -> str:
    ...
    parts.append(f"<title>AI情報収集 Top{total} / {date}</title>")
    parts.append(f"<style>{CSS}</style></head><body><div class='container'>")
    parts.append("<header>")
    parts.append("<h1>AI情報収集</h1>")
    ...
    if not items:
        ...
        parts.append("<footer>AI情報収集</footer></div></body></html>")
        return "".join(parts)
    ...
    parts.append(render_support_sns_section(load_support_sns()))
    parts.append("<footer>AI情報収集 / Generated by Claude</footer>")
    parts.append("</div>")
...
def render_archive(dates: list[str]) -> str:
    ...
    parts.append("<!doctype html><html lang='ja'><head><meta charset='utf-8'>")
    parts.append("<meta name='viewport' content='width=device-width,initial-scale=1'>")
    parts.append("<title>AI情報収集 — 過去ログ</title>")
    parts.append(f"<style>{CSS}</style></head><body><div class='container'>")
    parts.append("<header>")
