...
class AiAppSelfbuildOfferTests(unittest.TestCase):
    ...
    def test_homepage_merges_paid_consultation_into_one_selfbuild_course(self) -> None:
        ...
        self.assertIn("<h3>AIアプリサイト自作講習・相談</h3>", self.page)
        self.assertIn("相談だけで終わらず、自分で作って直せるようになる。", self.page)
    ...
    def test_ai_app_site_is_an_advice_page_for_building_it_yourself(self) -> None:
        ...
        self.assertIn("AIアプリサイトを、<br>自分で作れるように。", rendered)
        self.assertIn("11,000円", rendered)
        self.assertIn("120分", rendered)
        self.assertIn("自分で確認し、直し、公開できる", rendered)
        self.assertIn(SELFBUILD_SQUARE_URL, rendered)
        self.assertNotIn("99,000円", rendered)
        self.assertNotIn("198,000円", rendered)
        self.assertNotIn("500,000円", rendered)
