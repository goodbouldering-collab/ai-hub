...
class RenderedSalonTest(unittest.TestCase):
    ...
    def test_salon_is_one_complete_menu_before_the_venue_map(self) -> None:
        ...
        simple_head = self.html.index("class='salon-simple-head'")
        checkout = self.html.index("class='salon-register-form salon-simple-form'", simple_head)
        details = self.html.index("class='salon-all-details salon-all-details--complete'", checkout)
        visual = self.html.index("class='salon-main-visual'", details)
        self.assertLess(simple_head, checkout)
        self.assertLess(checkout, details)
        self.assertLess(details, visual)
